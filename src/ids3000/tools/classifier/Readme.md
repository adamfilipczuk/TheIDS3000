...
Use a `conn.log.labeled` file as your base. You may add more data to the fileif you would like to train on multiple dataset objects, just copy the data, leaving the headers behind and paste at the bottom of your base `conn.log.labeled` file. Check the headers to make sure all headers are tab-delineated. If the headers are delineated by spaces '   ' instead of tabs '    ', delete the spaces and replace with a single tab. You may then save this file to `./src/ids3000/tools/classifier/data/conn.log.labeled`.
